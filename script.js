...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Deshabilitar auto-rotate en dispositivos móviles
    if (isMobile && modelViewer.hasAttribute('auto-rotate')) {
    ...
    function toggleDimensions() {
        ...
        if (dimensionsVisible) {
            ...
            // Solo restaurar el zoom y pan si no estamos en móvil (para mantener la configuración original)
            if (!isMobile) {
    ...
    // Optimizar el renderizado del modelo 3D
    function optimizeModel() {
        // Reducir la calidad de render en móviles
        if (isMobile) {
            if (modelViewer.model) {
                // Establecer menor frecuencia de actualización
                const reduceUpdateRate = () => {
                    if (dimensionsVisible) return;
                    
                    // Reducir la tasa de renderizado cuando no está interactuando
                    ...
                    const checkInteractionStatus = () => {
                        ...
                        // Si ha pasado tiempo sin interacción, reducir framerate
                        if (timeSinceInteraction > 3000) { // 3 segundos
                            modelViewer.setAttribute('min-frame-visibility', '0');
                            modelViewer.setAttribute('max-frame-visibility', '0.5');
                        }
                    };
                    
                    // Reiniciar el temporizador cuando hay interacción
                    modelViewer.addEventListener('camera-change', () => {
                        lastInteraction = Date.now();
                        modelViewer.setAttribute('min-frame-visibility', '0');
                        modelViewer.setAttribute('max-frame-visibility', '1');
                    });
                    
                    renderInterval = setInterval(checkInteractionStatus, 1000);
                };
                
                // Aplicar optimizaciones cuando el modelo está cargado
                if (modelViewer.loaded) {
                    reduceUpdateRate();
                } else {
                    modelViewer.addEventListener('load', reduceUpdateRate);
                }
            }
        }
    }
